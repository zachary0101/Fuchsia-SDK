...
def _GetStrippedPath(bin_path):
  ...
  output directory."""
  stripped_path = bin_path.replace('lib.unstripped/', 'lib/').replace(
      'exe.unstripped/', '')
  if os.path.exists(stripped_path):
    return stripped_path
  else:
    shutil.copyfile(bin_path, stripped_path)
    return stripped_path
...
def _WriteBuildIdsTxt(binary_paths, ids_txt_path):
  ...
  with open(ids_txt_path, 'w') as ids_file:
    ...
    stripped_binary_paths = set(map(_GetStrippedPath, binary_paths))
...
def ParseComponent(files):
  component_info = []
  for f in files:
    component_info += json.load(open(f, 'r'))
  return component_info

def GetAppFilename(component_info):
  for c in component_info:
    if c.get('type') == 'dep':
      pos = c.get('source').find(':')
      return c.get('source')[pos+1:]

def GetManifestFile(component_info):
  for c in component_info:
    if c.get('type') == 'manifest':
      return str(c.get('source'))

def BuildManifest(args):
  component_info = ParseComponent(args.json_file)
  binaries = []
  app_filename = GetAppFilename(component_info)
  with open(args.manifest_path, 'w') as manifest, \
    ...
    cmx_source = GetManifestFile(component_info)
...
def main():
  ...
  parser.add_argument('--json-file', required=True, action='append' )
