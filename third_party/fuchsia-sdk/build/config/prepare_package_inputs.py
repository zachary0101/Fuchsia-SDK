...
def _IsBinary(path):
    """Checks if the file at |path| is an ELF executable by inspecting its FourCC header.
  """
...
def ParseComponent(component_info_file):
...
def BuildManifest(args):
    component_info_list = ParseComponent(args.json_file)
    binaries = []
